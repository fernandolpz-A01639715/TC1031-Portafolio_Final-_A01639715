...
class HashTable {
  ...
  public:
    HashTable();
    ~HashTable();
    void setMaxCapacity(int selectedMaxSize);
...
template<class K, class T>
void HashTable<K, T>::setMaxCapacity(int selectedMaxSize) {
...
template<class K, class T>
unsigned int HashTable<K, T>::getNumCollisions() {
...
void HashTable<K,T>::add(K keyVal, T value) {
  ...
  if (node.getStatus() != 1) { // Cell is free
    ...
    numCollisions += i; // se suman las colisiones de este add
